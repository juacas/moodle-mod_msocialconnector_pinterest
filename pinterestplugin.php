...
//                 }
    ...
    public function render_user_linking($user) {
        ...
        if ($socialids == null) { // Offer to register.
            $pixurl = new \moodle_url('/mod/msocial/connector/pinterest/pix');
            $userfullname = fullname($user);
            if ($USER->id == $user->id) {
                $urlprofile = new \moodle_url('/mod/msocial/connector/pinterest/pinterestSSO.php',
                        array('id' => $cm->id, 'action' => 'connect', 'type' => 'profile'));
                $usermessage = get_string('no_pinterest_name_advice2', 'msocialconnector_pinterest',
                        ['userfullname' => $userfullname, 'userid' => $USER->id, 'courseid' => $course->id,
                                        'url' => $urlprofile->out(false), 'pixurl' => $pixurl->out(false)]);
            } else {
                $usermessage = get_string('no_pinterest_name_advice', 'msocialconnector_pinterest',
                        ['userfullname' => $userfullname, 'userid' => $user->id, 'courseid' => $course->id,
                                        'pixurl' => $pixurl->out()]);
            }
        } else {
